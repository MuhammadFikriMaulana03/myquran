...
export default function KiblatPage() {
  ...
  // Kalkulasi Arah & Toleransi Tepat Kiblat
  ...
  // Logika Mencari Tahu Apakah Arah Sudah Pas (Toleransi 2 Derajat)
  let angleDifference = Math.abs(realHeadingText - qiblaTarget);
  if (angleDifference > 180) angleDifference = 360 - angleDifference; // Koreksi putaran 360

  const isQiblaCorrect = qiblaBearing !== null && compassHeading !== null && angleDifference <= 2;

  // Efek Getar Saat Pertama Kali Masuk Zona Akurat
  useEffect(() => {
    if (isQiblaCorrect && typeof navigator !== 'undefined' && navigator.vibrate) {
      // Getar 2 kali cepat (100ms nyala, 50ms mati, 100ms nyala)
  ...
  return (
    <main className="container mx-auto p-4 md:p-8 max-w-md min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 flex flex-col items-center">
      {/* CSS Animasi 3D & Penyembunyi Dark Mode */}
      <style>{`
        button[title="Ubah Tema"] { display: none !important; }
      ...
      {/* HEADER DINAMIS (Warna Teks Berubah Saat Tepat Arah) */}
      <div className="text-center mb-8 transition-colors duration-500">
        <h1
          className={`text-3xl font-extrabold mb-1 tracking-tight transition-colors duration-500 ${isQiblaCorrect ? 'text-emerald-500 dark:text-emerald-400 drop-shadow-[0_0_10px_rgba(16,185,129,0.5)]' : 'text-emerald-800 dark:text-emerald-400'}`}
        >
          Qibla <span className="font-light">Compass</span>
        </h1>
        {currentStep === 'COMPASS' ? (
          <p className={`text-xs uppercase tracking-widest font-bold transition-all duration-300 ${isQiblaCorrect ? 'text-emerald-500 scale-105' : 'text-slate-500'}`}>
            {isQiblaCorrect ? '✨ Arah Kiblat Tepat ✨' : 'Putar badan Anda perlahan...'}
          </p>
        ) : (
      ...
      {/* TAHAP 1 & TAHAP 2 SAMA SEPERTI SEBELUMNYA ... */}
      ...
      {/* TAHAP 3: KOMPAS AKTIF DENGAN FITUR GLOWING */}
      {currentStep === 'COMPASS' && !error && (
        <div className="flex flex-col items-center w-full animate-in fade-in duration-700">
          <div className="relative w-[280px] h-[280px] sm:w-[320px] sm:h-[320px] flex items-center justify-center my-8">
            {/* CINCIN UTAMA KOMPAS (Menyala Hijau Saat Tepat) */}
            <div
              className={`absolute inset-0 rounded-full flex items-center justify-center transition-all duration-500 ease-linear ${
                isQiblaCorrect
                  ? 'bg-emerald-50 dark:bg-emerald-950/40 border-[8px] border-emerald-400 dark:border-emerald-500 shadow-[0_0_60px_rgba(16,185,129,0.6)] scale-105'
                  : 'bg-gradient-to-b from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-950 border-[6px] border-emerald-50 dark:border-slate-800 shadow-[0_0_40px_rgba(16,185,129,0.15)]'
              }`}
            ...
            >
              ...
              <span className={`absolute top-2 font-black text-xl drop-shadow-md ${isQiblaCorrect ? 'text-emerald-600 dark:text-emerald-400' : 'text-emerald-600 dark:text-emerald-500'}`}>N</span>
              ...
              {/* JARUM KIBLAT (Ikon menyala saat tepat) */}
              <div className="absolute inset-0 flex items-center justify-center transition-transform duration-700 ease-out z-10" style={{ transform: `rotate(${qiblaTarget}deg)` }}>
                <div className="flex flex-col items-center transform -translate-y-[4.8rem]">
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center mb-1.5 border-2 z-10 transition-colors duration-300 ${
                      isQiblaCorrect ? 'bg-amber-500 border-amber-200 shadow-[0_0_20px_rgba(245,158,11,0.8)]' : 'bg-emerald-600 dark:bg-emerald-500 border-white dark:border-slate-900 shadow-lg shadow-emerald-600/40'
                    }`}
                  >
                    <svg viewBox="0 0 24 24" className="w-5 h-5 text-white" fill="currentColor">
                      <path d="M5 4h14v16H5z" />
                      <path d="M5 9h14v2H5z" fill={isQiblaCorrect ? '#FFF' : '#D4AF37'} />
                    </svg>
                  </div>
                  <div
                    className={`w-1.5 h-16 rounded-full transition-colors duration-300 ${isQiblaCorrect ? 'bg-gradient-to-b from-amber-500 to-transparent' : 'bg-gradient-to-b from-emerald-600 to-transparent dark:from-emerald-500'}`}
                  ></div>
                </div>
              </div>
            </div>
            ...
            <div
              className={`w-4 h-4 rounded-full z-20 border-[3px] border-white dark:border-slate-900 transition-colors duration-300 ${
                isQiblaCorrect ? 'bg-amber-500 shadow-[0_0_15px_rgba(245,158,11,0.8)]' : 'bg-emerald-600 dark:bg-emerald-400 shadow-md'
              }`}
            ></div>
          </div>
          ...
          <div className="w-full flex flex-col gap-4 mt-2">
            ...
            {/* Kotak Informasi juga menyala saat pas! */}
            <div
              className={`backdrop-blur-xl p-5 rounded-3xl shadow-sm border w-full text-center transition-colors duration-500 ${
                isQiblaCorrect ? 'bg-emerald-50/90 dark:bg-emerald-900/30 border-emerald-300 dark:border-emerald-700' : 'bg-white/80 dark:bg-slate-900/80 border-slate-100 dark:border-slate-800'
              }`}
            >
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className={`font-black text-xl transition-colors duration-300 ${isQiblaCorrect ? 'text-amber-500 dark:text-amber-400' : 'text-slate-700 dark:text-slate-200'}`}>
                    {compassHeading !== null ? `${realHeadingText.toFixed(1)}°` : '--'}
                  </p>
                </div>
              </div>
