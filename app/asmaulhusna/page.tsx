...
import { useRouter } from 'next/navigation'; // 1. Import router dari next/navigation
  ...
  return (
    <main className="container mx-auto p-4 md:p-8 max-w-6xl min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 transition-colors duration-300">
      ...
      <div className="text-center mb-8">
        <h1 className="text-4xl font-extrabold text-emerald-700 dark:text-emerald-400 mb-2">Asmaul Husna</h1>
        <p className="text-slate-500 dark:text-slate-400">99 Nama Allah SWT yang Agung beserta artinya</p>
      </div>
      ...
      <div className="max-w-md mx-auto mb-10 relative">
        <div className="flex items-center w-full bg-white dark:bg-slate-900 p-2 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 focus-within:ring-2 focus-within:ring-emerald-400 transition-all">
          ...
          <input
            ...
            className="flex-1 bg-transparent text-slate-700 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 text-sm md:text-base outline-none px-2 py-2 w-full"
          />
          {searchQuery && (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {filteredData.length > 0 ? (
          filteredData.map((item) => (
            <div
              key={item.urutan}
              className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 hover:shadow-md hover:border-emerald-300 dark:hover:border-emerald-600 transition-all flex flex-col justify-between text-center group"
            >
              {/* Nomor Urut */}
              <div className="flex justify-between items-center mb-4">
                <span className="w-8 h-8 flex items-center justify-center bg-emerald-100 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-400 font-bold text-xs rounded-full border border-emerald-200 dark:border-emerald-900">
                  {item.urutan}
                </span>
                <span className="text-xs text-slate-400 dark:text-slate-500 font-medium">Asmaul Husna</span>
              </div>

              {/* Teks Arab */}
              <div className="my-3">
                <h3 className="text-3xl font-serif text-emerald-800 dark:text-emerald-400 group-hover:scale-105 transition-transform">{item.arab}</h3>
              </div>

              {/* Latin & Arti */}
              <div className="pt-4 border-t border-slate-50 dark:border-slate-800 mt-2">
                <p className="font-bold text-slate-700 dark:text-slate-200 text-base mb-1">{item.latin}</p>
                <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">{item.arti}</p>
              </div>
            </div>
          ))
        ) : (
          <div className="col-span-full text-center p-12 bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 text-slate-500 dark:text-slate-400">
