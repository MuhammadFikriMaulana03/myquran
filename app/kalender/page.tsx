...
export default async function KalenderPage(props: PageProps) {
  ...
  return (
    <main className="container mx-auto p-4 md:p-8 max-w-6xl min-h-screen bg-slate-50 text-slate-800">
      ...
      {/* Header Kalender (Teks Bulan & Tahun Diperbesar) */}
      <div className="text-center mb-8">
        <h1 className="text-3xl md:text-5xl font-extrabold text-emerald-800 mb-2 capitalize">
          {kalender.namaBulan} {kalender.tahun}
        </h1>
        <p className="text-slate-500 uppercase tracking-widest font-semibold text-xs md:text-sm">Kalender Islami & Nasional</p>
      </div>
      ...
      {/* HEADER NAMA HARI */}
      <div className="grid grid-cols-7 gap-3 mb-3 text-center font-bold text-emerald-800 text-sm md:text-base">
        <div className="p-2 bg-emerald-100/60 rounded-xl text-red-600">Min</div>
        <div className="p-2 bg-emerald-100/60 rounded-xl">Sen</div>
        <div className="p-2 bg-emerald-100/60 rounded-xl">Sel</div>
        <div className="p-2 bg-emerald-100/60 rounded-xl">Rab</div>
        <div className="p-2 bg-emerald-100/60 rounded-xl">Kam</div>
        <div className="p-2 bg-emerald-100/60 rounded-xl">Jum</div>
        <div className="p-2 bg-emerald-100/60 rounded-xl">Sab</div>
      </div>
      ...
      <div className="grid grid-cols-7 gap-3">
        ...
        {kalender.hari.map((item: any, index: number) => {
          ...
          return (
            <div
              key={index}
              className={`p-3 md:p-4 rounded-2xl border transition-all flex flex-col justify-between min-h-[130px] ${
                !isBulanAktif
            ...
            >
              <div>
                <div className="flex justify-between items-start mb-2">
                  <span className={`text-2xl md:text-3xl font-bold ${!isBulanAktif ? 'text-slate-400' : isTanggalMerah ? 'text-red-600' : 'text-slate-700'}`}>{masehi.day}</span>

                  <span className={`text-xs font-bold px-1.5 py-0.5 rounded-md ${!isBulanAktif ? 'bg-slate-200 text-slate-500' : 'text-emerald-700 bg-emerald-100'}`}>{hijriah.day}</span>
                </div>

                <div className={`text-xs font-bold leading-tight ${!isBulanAktif ? 'text-slate-400' : 'text-emerald-600'}`}>
              ...
              {isBulanAktif && (
                <div className="space-y-1 mt-2">
                  {liburNasionalName && <div className="text-[9px] md:text-[10px] font-bold text-red-700 bg-red-100 px-1.5 py-1 rounded text-center leading-tight">🔴 {liburNasionalName}</div>}

                  {adaMomenIslam && (
                    <div>
                      {hijriah.holidays.map((namaLibur: string, hIndex: number) => (
                        <div key={hIndex} className="text-[9px] md:text-[10px] font-bold text-emerald-800 bg-emerald-200/60 px-1.5 py-1 rounded text-center leading-tight">
